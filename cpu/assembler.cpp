...
namespace trios {
    ...
    void Assembler::jmp(Tryte address) {
        memory.saveInstruction(this->address, {Tryte((int)Opcodes::JMP), address, Tryte(0)});
        this->address = this->address + Tryte(4);
    }

    void Assembler::jz(Tryte address) {
        memory.saveInstruction(this->address, {Tryte((int)Opcodes::JZ), address, Tryte(0)});
        this->address = this->address + Tryte(4);
    }

    void Assembler::call(Tryte address) {
        memory.saveInstruction(this->address, {Tryte((int)Opcodes::CALL), address, Tryte(0)});
        this->address = this->address + Tryte(4);
    }

    void Assembler::ret() {
        memory.saveInstruction(this->address, {Tryte((int)Opcodes::RET), Tryte(0), Tryte(0)});
        this->address = this->address + Tryte(4);
    }
